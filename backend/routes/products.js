...
module.exports = (db) => {
  ...
  router.get('/search', (req, res) => {
    ...
    const query = 'SELECT * FROM products WHERE name LIKE ? OR brand LIKE ?';
        ...
        db.run(query, values, function(err) {
          ...
          if (stock !== undefined && product.stock !== stock) {
            db.run(
              ...
              [id, product.stock, stock, new Date().toISOString(), req.headers['user-agent'] || 'Unknown']
            );
          }
    ...
    fs.createReadStream(filePath)
      ...
      .on('end', () => {
        ...
        products.forEach((product) => {
          ...
          if (!name) {
            skipped.push({ product, reason: 'Missing name' });
            processed++;
            checkComplete();
            return;
          }
          ...
          db.get('SELECT id FROM products WHERE name = ?', [name], (err, existing) => {
            if (err) {
              skipped.push({ product, reason: err.message });
              processed++;
              checkComplete();
              return;
            }
            ...
            if (existing) {
              skipped.push({ product, reason: 'Duplicate name' });
              processed++;
              checkComplete();
              return;
            }
            ...
            db.run(
              ...
              function(insertErr) {
                if (insertErr) {
                  skipped.push({ product, reason: insertErr.message });
                } else {
